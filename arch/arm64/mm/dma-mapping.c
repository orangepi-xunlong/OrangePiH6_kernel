...
static void *__alloc_from_pool(size_t size,
	struct page **ret_pages, gfp_t flags)
{
	unsigned long val;
	void *ptr = NULL;
				     ...
				     struct dma_attrs *attrs)
{
	...
	if (dma_get_attr(DMA_ATTR_NO_KERNEL_MAPPING, attrs)) {
		...
		page1 = virt_to_page(ptr);
		coherent_ptr = dma_common_contiguous_remap(page1, size,
				VM_USERMAP,
...
static inline void covert_bitmap(unsigned long *buf, unsigned int buflen,
			struct dma_iommu_mapping *mapping)
{
	unsigned long *bitmap_buf = (unsigned long *)mapping->bitmap;
	unsigned int bitmap_long_num = BITS_TO_LONGS(mapping->bits);
	unsigned int i;

	for (i = 0; i < bitmap_long_num; i++)
		buf[i / BITS_PER_LONG] |=
			(bitmap_buf[i] != 0) << (i % BITS_PER_LONG);
}

static inline void print_hex(unsigned int *buf, unsigned int len)
{
	int i;
	...
	for (i = 0; i < len >> 3; i++) {
		...
		buf[8*i+0], buf[8*i+1], buf[8*i+2], buf[8*i+3],
		buf[8*i+4], buf[8*i+5], buf[8*i+6], buf[8*i+7]);
	}
}

struct master_iova_info {
	struct device *dev;
...
static struct master_iova_info master_head = {
	.dev = NULL,
	.iova_addr = 0,
	.size = 0,
	.list = LIST_HEAD_INIT(master_head.list),
};

DEFINE_MUTEX(master_lock);
#endif
static inline dma_addr_t __alloc_iova(struct device *dev,
	struct dma_iommu_mapping *mapping, size_t size)
{
	...
	if (start > mapping->bits) {
...
#ifdef CONFIG_DMA_API_DEBUG
		{
			...
			unsigned int bitmap_size =
				(BITS_TO_LONGS(mapping->bits) * sizeof(long)) /
								BITS_PER_LONG;
			struct master_iova_info *master;

			bitmap_buf = kzalloc(bitmap_size, GFP_KERNEL);
			if (!bitmap_buf)
				return DMA_ERROR_CODE;

			pr_err("%s failed:0x%lx\n", __func__, size);
			list_for_each_entry(master, &master_head.list, list) {
				pr_err("%s, iova:0x%llx, size:0x%lx\n",
					dev_name(master->dev),
					master->iova_addr, master->size);
			}
			covert_bitmap((unsigned long *)bitmap_buf,
				bitmap_size, mapping);
			print_hex((unsigned int *)bitmap_buf,
				bitmap_size / sizeof(int));
			kzfree(bitmap_buf);
		}
#endif
		return DMA_ERROR_CODE;
...
#ifdef CONFIG_DMA_API_DEBUG
	{
		...
		master = kmalloc(sizeof(struct master_iova_info), GFP_KERNEL);
		if (master) {
			master->dev = dev;
			master->iova_addr = mapping->base + (start << PAGE_SHIFT);
			master->size = size;
			mutex_lock(&master_lock);
			list_add(&master->list, &master_head.list);
			       ...
			       dma_addr_t addr, size_t size)
{
	...
	{
		struct master_iova_info *master;
		struct master_iova_info *tmp;

		mutex_lock(&master_lock);
		list_for_each_entry_safe(master, tmp, &master_head.list, list) {
			if (master->iova_addr == addr) {
					...
					struct dma_attrs *attrs)
{
	...
	dma_addr = __alloc_iova(dev, mapping, size);
			  ...
			  bool is_coherent)
{
	...
	*handle = DMA_ERROR_CODE;
	...
	iova = __alloc_iova(dev, mapping, size);
	if (iova == DMA_ERROR_CODE) {
		pr_err("%s, %d, __alloc_iova failed\n", __func__, __LINE__);
		return -ENOMEM;
	}
	     ...
	     enum dma_data_direction dir, struct dma_attrs *attrs)
{
	...
	dma_addr = __alloc_iova(dev, mapping, len);
	if (dma_addr == DMA_ERROR_CODE)
		return dma_addr;
