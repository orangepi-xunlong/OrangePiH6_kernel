...
u8 dbg_etf     = XRADIO_DBG_ALWY|XRADIO_DBG_ERROR|XRADIO_DBG_WARN;
	...
	char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	char buf[256];
	size_t size = 0;
	sprintf(buf, "irq_count=%d, rx_total=%d, miss=%d, fix=%d, next=%d, burst=%d"
		"tx_total=%d, tx_burst=%d\n",
		irq_count, rx_total_cnt, int_miss_cnt,
		fix_miss_cnt, next_rx_cnt, hw_priv->debug->rx_burst,
		tx_total_cnt, hw_priv->debug->tx_burst);
	size = strlen(buf);
...
#define ERR_LINE  do { line = __LINE__; goto err; } while (0)
	...
	if (!debugfs_create_x32("set_sdio_clk", S_IRUSR | S_IWUSR,
