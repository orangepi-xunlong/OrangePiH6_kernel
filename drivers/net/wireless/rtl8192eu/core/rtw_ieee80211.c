...
#define DEFAULT_RANDOM_MACADDR 1
	...
	if (rtw_check_invalid_mac_address(mac, _TRUE) == _FALSE)
		DBG_871X_LEVEL(_drv_always_, "MAC Address from custom burning is "
				""MAC_FMT"\n", MAC_ARG(mac));
	else
#endif
...
err_chk:
	if (rtw_check_invalid_mac_address(mac, _TRUE) == _TRUE) {
		...
		if (!is_valid_ether_addr(mac)) {
			...
			DBG_871X_LEVEL(_drv_always_, "MAC Address from random mac is "
					""MAC_FMT"\n", MAC_ARG(mac));
		} else {
			DBG_871X_LEVEL(_drv_always_, "MAC Address from chipid is "
					""MAC_FMT"\n", MAC_ARG(mac));
		}
#else
