	...
	)
{
	...
	odm_set_rf_reg(p_dm_odm, (enum odm_rf_radio_path_e)path, add, RFREGOFFSETMASK, data);

	for (i = 0; i < 100; i++) {
		if (odm_get_rf_reg(p_dm_odm, (enum odm_rf_radio_path_e)path, add, RFREGOFFSETMASK) == data)
			break;
		else {
			ODM_delay_us(10);
			odm_set_rf_reg(p_dm_odm, (enum odm_rf_radio_path_e)path, add, RFREGOFFSETMASK, data);
...
)
{
	...
	/*0xdf[4]=0*/
	_iqk_rf_set_check(p_dm_odm, ODM_RF_PATH_A, 0xdf, RF_backup[0][ODM_RF_PATH_A] & (~BIT(4)));
	_iqk_rf_set_check(p_dm_odm, ODM_RF_PATH_B, 0xdf, RF_backup[0][ODM_RF_PATH_B] & (~BIT(4)));
...
)
{
	...
	for (path = 0; path < 2; path++) {
		if (reload_idx == 2) {
			...
			tmp = odm_get_rf_reg(p_dm_odm, (enum odm_rf_radio_path_e)path, 0xdf, RFREGOFFSETMASK) | BIT(4);
			_iqk_rf_set_check(p_dm_odm, (enum odm_rf_radio_path_e)path, 0xdf, tmp);			
...
/*IQK_version:0x2a NCTL:0x8*/
/*1.write&check RF 0xdf*/
void
