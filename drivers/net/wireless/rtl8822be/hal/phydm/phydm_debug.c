...
)
{
	...
	case PHYDM_ADAPTIVE_SOML:
		...
		if (input_idx >= 1)
			phydm_soml_debug(p_dm_odm, (u32 *)var1, &used, output, &out_len);
	...
	case PHYDM_DIS_DYM_ANT_WEIGHTING:	
		...
		if (var1[0] == 1) {
			p_dm_odm->is_disable_dym_ant_weighting = 1;
