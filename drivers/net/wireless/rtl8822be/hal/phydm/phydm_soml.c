...
)
{
	...
	if (swch == SOML_ON) {
		...
		PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("(( Turn on )) SOML\n"));
		...
		PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("(( Turn off )) SOML\n"));
...
)
{
	...
	PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("******SOML_Callback******\n"));
	phydm_adsl(p_dm);
...
)
{
	...
	if ((p_dm->support_ic_type & ODM_ADAPTIVE_SOML_SUPPORT_IC)) {
		if (p_dm->is_one_entry_only) {
			PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("rssi_min =%d\n", p_dm->rssi_min));
			PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("soml_state_cnt =((%d))\n", p_dm_soml_table->soml_state_cnt));
			...
			/*Decision state: ==============================================================*/
				...
				if (p_dm->support_ic_type == ODM_RTL8197F) {
					...
					for (i = 0; i < rate_num; i++) {
						rate_ss_shift = (i << 3);
						PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("*num_ht_bytes_on  HT MCS[%d :%d ] = {%d, %d, %d, %d, %d, %d, %d, %d}\n",
					...
					for (i = 0; i < rate_num; i++) {
						rate_ss_shift = (i << 3);
						PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("*num_ht_bytes_off  HT MCS[%d :%d ] = {%d, %d, %d, %d, %d, %d, %d, %d}\n",
							(rate_ss_shift), (rate_ss_shift+7),
					...
					for (i = 0; i < rate_num; i++) {
						rate_ss_shift = 10 * i;
						PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("* num_vht_bytes_on  VHT-%d ss MCS[0:9] = {%d, %d, %d, %d, %d, %d, %d, %d, %d, %d}\n",
							(i + 1),
					...
					for (i = 0; i < rate_num; i++) {
						rate_ss_shift = 10 * i;
						PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("* num_vht_bytes_off  VHT-%d ss MCS[0:9] = {%d, %d, %d, %d, %d, %d, %d, %d, %d, %d}\n",
							(i + 1),
				...
				/* [Decision] */
				PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("[  byte_total_on = %d ; byte_total_off = %d ]\n", byte_total_on, byte_total_off));
				PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("[Decisoin state ]\n"));
				if (byte_total_on > byte_total_off) {
					next_on_off = SOML_ON;
					PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("[ byte_total_on > byte_total_off ==> SOML_ON ]\n"));
				} else if (byte_total_on < byte_total_off) {
					next_on_off = SOML_OFF;
					PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("[ byte_total_on < byte_total_off ==> SOML_OFF ]\n"));
				} else {
					PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("[ stay at soml_last_state ]\n"));
					next_on_off = p_dm_soml_table->soml_last_state;
				}

				PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("[ Final decisoin ] : "));
				phydm_soml_on_off(p_dm, next_on_off);
				p_dm_soml_table->soml_last_state = next_on_off;
			}
		} else {
			PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("[multi-Client]\n"));
...
)
{
	...
	if (!(p_dm->support_ability & ODM_BB_ADAPTIVE_SOML)) {
		PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("[Return]   Not Support Adaptive SOML\n"));
		return;
	}
	PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("phydm_adaptive_soml_init\n"));
...
)
{
	...
	if (!(p_dm->support_ability & ODM_BB_ADAPTIVE_SOML)) {
		PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML,
	...
	if (p_dm_soml_table->soml_select == 0) {
		PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("[Adaptive SOML Training !!!]\n"));
	} else if (p_dm_soml_table->soml_select == 1) {
		PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("[Turn on SOML !!!] Exit from Adaptive SOML Training\n"));
		phydm_soml_on_off(p_dm, SOML_ON);
		return;
	} else if (p_dm_soml_table->soml_select == 2) {
		PHYDM_DBG(p_dm, ODM_COMP_ADAPTIVE_SOML, ("[Turn off SOML !!!] Exit from Adaptive SOML Training\n"));
