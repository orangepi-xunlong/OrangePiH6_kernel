...
)
{
	...
	if (*p_dm_odm->p_channel <= 14) {
		if ((switch_soml == true) && (!((p_dm_odm->rfe_type == 0) || (p_dm_odm->rfe_type == 3) || (p_dm_odm->rfe_type == 5) || (p_dm_odm->rfe_type == 8) || (p_dm_odm->rfe_type == 10) || (p_dm_odm->rfe_type == 12) || (p_dm_odm->rfe_type == 13) || (p_dm_odm->rfe_type == 14)))) {
			odm_set_bb_reg(p_dm_odm, 0x8cc, MASKDWORD, 0x08100000);
			odm_set_bb_reg(p_dm_odm, 0x8d8, BIT(27), 0x0);
		}
...
)
{
	...
	if (*p_dm_odm->p_channel <= 14) {
		if (p_dm_odm->rssi_min >= rssi_l2h) {
			...
			PHYDM_DBG(p_dm_odm, ODM_COMP_COMMON, ("Equal weighting ,rssi_min = %d\n, 0xf94[2:0] = 0x%x\n", p_dm_odm->rssi_min, reg_8));
		} else if (p_dm_odm->rssi_min <= rssi_h2l) {
			odm_set_bb_reg(p_dm_odm, 0x98c, MASKDWORD, 0x43440000);
	
			/*fix sec_min_wgt = 1/2*/
			reg_8 = (u8)odm_get_bb_reg(p_dm_odm, 0xf94, BIT(0)|BIT(1)|BIT(2));
			PHYDM_DBG(p_dm_odm, ODM_COMP_COMMON, ("AGC weighting ,rssi_min = %d\n, 0xf94[2:0] = 0x%x\n", p_dm_odm->rssi_min, reg_8));
        }
	} else {
			odm_set_bb_reg(p_dm_odm, 0x98c, MASKDWORD, 0x43440000);
...
)
{
	...
	if((p_dm_odm->p_advance_ota & PHYDM_HP_OTA_SETTING_A) || (p_dm_odm->p_advance_ota & PHYDM_HP_OTA_SETTING_B)) {
		...
		else if (p_dm_odm->bhtstfdisabled == 1)
			ODM_RT_TRACE(p_dm_odm, ODM_PHY_CONFIG, ODM_DBG_LOUD, ("Dynamic HTSTF is disable\n"));
        }
