					...
					__u32 freq, __u32 relation)
{
	...
	arisc_dvfs_set_cpufreq(freq, ARISC_DVFS_PLL1, ARISC_DVFS_ASYN, NULL, NULL);

	/* CPUS max latency is 5ms, so CPU need > 5ms. 20170703*/
	timeout = 1000;
	while (timeout-- && (clk_get_rate(sunxi_cpufreq.clk_pll) != freq*1000))
		udelay(10);

	/* when the task scheduling; need to check again */
	if (clk_get_rate(sunxi_cpufreq.clk_pll) != freq*1000)
#endif
	{
