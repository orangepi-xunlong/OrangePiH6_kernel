...
static void post2_cb_0(struct work_struct *work)
{
	...
	if ((composer_priv.reg_active[0] ||
	     (bsp_disp_get_output_type(0) == DISP_OUTPUT_TYPE_NONE))
	    && (composer_priv.reg_active[1] ||
		(bsp_disp_get_output_type(1) == DISP_OUTPUT_TYPE_NONE))) {
		imp_finish_cb(0);
	}
...
int dispc_gralloc_queue(struct setup_dispc_data_t *psDispcData,
			int ui32DispcDataLength, void (*cb_fn) (void *, int),
			void *cb_arg)
{
		...
		if (!psDispcData->show_black[disp]) {
			if (disp == 0) {
				...
				start_idx =
				    psDispcData->primary_display_layer_num;
				layer_num =
				    psDispcData->post2_layers -
				    psDispcData->primary_display_layer_num;
			}

			if ((layer_num > 1) &&
			    (psDispcData->layer_info[start_idx + 1].zorder <
			     psDispcData->layer_info[start_idx].zorder))
				haveFbTarget = 1;
			...
			if (haveFbTarget) {
				/* save fb layer info */
				memcpy(&layer_info,
				       &psDispcData->layer_info[start_idx],
				       sizeof(disp_layer_info));
				/* order other layer */
				for (i = 0; i < layer_num - 1; i++)
					memcpy(&psDispcData->
					       layer_info[start_idx + i],
					       &psDispcData->
					       layer_info[start_idx + i + 1],
					       sizeof(disp_layer_info));

				/* restore fb layer info at the last element */
				memcpy(&psDispcData->layer_info[start_idx + i],
				       &layer_info, sizeof(disp_layer_info));
			}
			...
			for (i = 0; i < 4; i++) {
				...
				if (layer_info.mode ==
					...
					if ((i >= layer_num)
					    || (psDispcData->layer_info[start_idx + i].mode ==
						DISP_LAYER_WORK_MODE_NORMAL))
						bsp_disp_layer_set_info(disp,
							hdl,
							&psDispcData->layer_info[start_idx + i]);
		...
		if (!psDispcData->show_black[disp]) {
			...
			for (i = 0; i < 4; i++) {
				...
				if (i < layer_num) {
					...
					if (layer_info.fb.format ==
					    DISP_FORMAT_YUV420_P) {
						layer_info.fb.addr[2] =
						    layer_info.fb.addr[0] +
						    layer_info.fb.size.width *
						    layer_info.fb.size.height;
						layer_info.fb.addr[1] =
						    layer_info.fb.addr[2] +
						    (layer_info.fb.size.width *
						     layer_info.fb.size.
						     height) / 4;
					}
					bsp_disp_layer_set_info(disp, hdl,
								&layer_info);
...
struct dispc_data_list_t {
	struct list_head list;
	struct setup_dispc_data_t hwc_data;
};
...
static int dispc_update_regs(struct setup_dispc_data_t *psDispcData)
{
	disp_layer_info layer_info;
	int i, disp, hdl;
	int start_idx, layer_num = 0;
	...
	for (i = 0; i < psDispcData->post2_layers; i++) {
		...
		if (psDispcData->acquireFence[i]) {
			err = sync_fence_wait(psDispcData->acquireFence[i],
					      1000);
			...
			if (err < 0) {
				__wrn
				    ("synce_fence_wait timeout,layer%d,fd:%d\n",
...
{
	struct dispc_data_list_t *data, *next;
...
static int hwc_commit(int sel, struct setup_dispc_data_t *disp_data)
{
	struct dispc_data_list_t *disp_data_list;
	...
	for (i = 0; i < disp_data->post2_layers; i++) {
		...
		if (disp_data->acquireFenceFd[i] >= 0) {
			disp_data->acquireFence[i] =
			    sync_fence_fdget(disp_data->acquireFenceFd[i]);
			if (!disp_data->acquireFence[i]) {
				pr_warn
				    ("sync_fence_fdget()fail, layer:%d fd:%d\n",
...
/* if(composer_priv.b_no_output == 0) */
	if (1) {
		...
		pt = sw_sync_pt_create(composer_priv.timeline,
				       composer_priv.timeline_max);
		fence = sync_fence_create("display", pt);
		sync_fence_install(fence, fd);

		disp_data_list =
		    kzalloc(sizeof(struct dispc_data_list_t), GFP_KERNEL);
		memcpy(&disp_data_list->hwc_data, disp_data,
		       sizeof(struct setup_dispc_data_t));
		/* spin_lock(&(composer_priv.update_reg_lock)); */
		mutex_lock(&(composer_priv.update_regs_list_lock));
		list_add_tail(&disp_data_list->list,
			      &composer_priv.update_regs_list);
		/* spin_unlock(&(composer_priv.update_reg_lock)); */
		mutex_unlock(&(composer_priv.update_regs_list_lock));
		...
		/*if(composer_priv.timeline_max > */
		/*      composer_priv.timeline->value) */
		/*{ */
		/*      mutex_lock(&(composer_priv.update_regs_list_lock)); */
		/*      sw_sync_timeline_inc(composer_priv.timeline, 1); */
		/*      mutex_unlock(&(composer_priv.update_regs_list_lock)); */
...
static int hwc_commit_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == DISP_CMD_HWC_COMMIT) {
		struct setup_dispc_data_t para;
...
static void hwc_commit_work(struct work_struct *work)
{
	...
	list_for_each_entry_safe(data, next, &saved_list, list) {
		...
		NumberOfLayer = data->hwc_data.layer_num[0] +
		    data->hwc_data.layer_num[1] + data->hwc_data.layer_num[2];
		for (i = 0; i < NumberOfLayer; i++) {
			if (AcquireFence[i]) {
				int err =
				    sync_fence_wait(AcquireFence[i], 1000);

				sync_fence_put(AcquireFence[i]);
				if (err < 0) {
					PRINTK
					    ("fence_wait timeout, Fence:%p\n",
					     AcquireFence[i]);
					composer_priv.cb_w_conut++;
					sw_sync_timeline_inc(composer_priv.
							     timeline, 1);
					goto free;
				}
			}
...
static int hwc_commit(struct setup_dispc_data_t *disp_data)
{
	struct dispc_data_list_t *disp_data_list;
	struct sync_fence *fence;
	struct sync_pt *pt;
	...
	if (composer_priv.b_no_output == 0) {
		if (disp_data->layer_num[0] + disp_data->layer_num[1] +
		    disp_data->layer_num[2] > 0) {
			...
			sync_fence_install(fence, fd);
			disp_data_list =
			    kzalloc(sizeof(struct dispc_data_list_t),
				    GFP_KERNEL);
			memcpy(&disp_data_list->hwc_data, disp_data,
			       sizeof(struct setup_dispc_data_t));
			mutex_lock(&(composer_priv.update_regs_list_lock));
			list_add_tail(&disp_data_list->list,
				      &composer_priv.update_regs_list);
			mutex_unlock(&(composer_priv.update_regs_list_lock));
			queue_work(composer_priv.Display_commit_work,
				   &composer_priv.commit_work);
		} else {
			kfree(AcquireFence);
		}
	} else {
...
{
	int ret = -1;
	struct setup_dispc_data_t *para = NULL;
	int fd[12] = { 0, };
	...
	if (cmd == DISP_CMD_HWC_COMMIT) {
		...
		para = kzalloc(sizeof(struct setup_dispc_data_t), GFP_KERNEL);
		...
		if (copy_from_user(fd, (void __user *)para->hConfigData,
				   12 * sizeof(int))) {
			PRINTK("copy_from_user fail\n");
			return -EFAULT;
		}
...
s32 disp_get_frame_count(u32 screen_id, char *buf)
{
	...
	current_index = (fcount_data.current_index == 0) ?
	    99 : (fcount_data.current_index - 1);
	...
	for (i = 0; i < 99; i++) {
		...
		if (fcount_data.frame_refresh_array[current_index]
		    < last_time) {
			count = i;
			break;
		}

		temp_time = fcount_data.frame_refresh_array[current_index]
		    - last_time;
	...
	PRINTK(KERN_ERR "[DISP] acquire %u freshrate %u release %u\n",
	       fcount_data.acquire_count, count, fcount_data.release_count);
	PRINTK(KERN_ERR "disp %u,video<%u,%u>, vsync<%u,%u,%u>, skip<%u,%u,%u>",
	       fcount_data.display_count, fcount_data.video_count,
	       fcount_data.video_strict_count, fcount_data.vsync_count[0],
	       fcount_data.vsync_count[1], fcount_data.vsync_count[2],
	       fcount_data.skip_count[0], fcount_data.skip_count[1],
	       fcount_data.skip_count[2]);

	return sprintf(buf,
		       "[DISP] acquire %u freshrate %u release %u,disp %u, video<%u,%u>, vsync<%u,%u,%u>, skip<%u,%u,%u>\n",
		       fcount_data.acquire_count, count,
		       fcount_data.release_count, fcount_data.display_count,
		       fcount_data.video_count, fcount_data.video_strict_count,
		       fcount_data.vsync_count[0], fcount_data.vsync_count[1],
		       fcount_data.vsync_count[2], fcount_data.skip_count[0],
		       fcount_data.skip_count[1], fcount_data.skip_count[2]);
}
...
static void disp_composer_proc(u32 sel)
{
	if ((sel == 0) ||
	    ((sel == 1) &&
	     (bsp_disp_get_output_type(0) == DISP_OUTPUT_TYPE_NONE) &&
	     (bsp_disp_get_output_type(2) == DISP_OUTPUT_TYPE_NONE)) ||
	    ((sel == 2) &&
	     (bsp_disp_get_output_type(0) == DISP_OUTPUT_TYPE_NONE))) {
		composer_priv.cur_count = composer_priv.cb_w_conut;
	}
...
static void imp_finish_cb(bool force_all)
{
	...
	while (composer_priv.timeline->value != composer_priv.cb_w_conut) {
		...
		if (!force_all && (composer_priv.timeline->value ==
				   composer_priv.cur_count - 1))
			break;
...
#endif
static int disp_calc_bandwidth(struct setup_dispc_data_t *psDispcData)
{
	disp_layer_info layer_info;
	int i, disp, hdl;
	int num_screens;
...
int dispc_gralloc_queue(struct setup_dispc_data_t *psDispcData)
{
	...
	for (disp = 0; disp < num_screens; disp++) {
		for (i = 0; i < 4; i++) {
			...
			if (layer_info.mode == DISP_LAYER_WORK_MODE_SCALER) {
				if ((i >= psDispcData->layer_num[disp]) ||
				    (psDispcData->layer_info[disp][i].mode ==
				     DISP_LAYER_WORK_MODE_NORMAL))
					bsp_disp_layer_set_info(disp, hdl,
								&psDispcData->
								layer_info[disp]
		...
		for (i = 0; i < 4; i++) {
			...
			if (i < psDispcData->layer_num[disp]) {
				...
				if (layer_info.fb.format >=
					...
					/* assume all yuv layer is video lyr */
					fcount_data.video_count++;
					bsp_disp_layer_get_info(disp, hdl,
							&layer_info_tmp);
					if (layer_info.fb.addr[0] !=
					    layer_info_tmp.fb.addr[0])
						fcount_data.
						    video_strict_count++;
				}
	...
	if (composer_priv.b_no_output == 1) {
		...
		for (disp = 0; disp < num_screens; disp++) {
			/* if specified disp have layer to be display, */
			/*then need to wait for it's vsync */
			composer_priv.reg_active[disp] =
			    (psDispcData->layer_num[disp] == 0) ? 1 : 0;
		}
	}
	mutex_unlock(&composer_priv.runtime_lock);
...
s32 composer_init(void)
{
	...
	memset(&composer_priv, 0x0, sizeof(struct composer_private_data));
	memset(&fcount_data, 0x0, sizeof(struct disp_fcount_data));
	INIT_WORK(&composer_priv.post2_cb_work, post2_cb);
	mutex_init(&composer_priv.runtime_lock);

	composer_priv.Display_commit_work =
	    create_freezable_workqueue("DisplayCommit");
	INIT_WORK(&composer_priv.commit_work, hwc_commit_work);
	INIT_LIST_HEAD(&composer_priv.update_regs_list);
	composer_priv.timeline = sw_sync_timeline_create("sunxi-fb");
	...
	spin_lock_init(&(composer_priv.update_reg_lock));
	disp_register_ioctl_func(DISP_CMD_HWC_COMMIT, hwc_commit_ioctl);
	disp_register_ioctl_func(DISP_CMD_HWC_GET_DISP_READY,
				 hwc_check_disp_ready);
	disp_register_sync_finish_proc(disp_composer_proc);
	disp_register_standby_func(hwc_suspend, hwc_resume);
	composer_priv.reg_active[0] = 1;
	composer_priv.reg_active[1] = 1;
	...
	if (disp_sys_script_get_item("disp_init", "disp_composer_mode",
				     &value, 1) == 1) {
