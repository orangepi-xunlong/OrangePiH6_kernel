...
static s32 lcd_clk_init(struct disp_lcd *lcd)
{
	...
	if (lcdp->sat_clk.clk &&
	   (__clk_get_enable_count(lcdp->sat_clk.clk) != 0))
		clk_disable(lcdp->sat_clk.clk);

	if (lcdp->extra_clk.clk &&
	   (__clk_get_enable_count(lcdp->extra_clk.clk) != 0))
		clk_disable(lcdp->extra_clk.clk);

	if (lcdp->merge_clk.clk &&
	   (__clk_get_enable_count(lcdp->merge_clk.clk) != 0))
...
static s32 disp_lcd_pwm_enable(struct disp_lcd *lcd)
{
	...
	if (disp_lcd_is_used(lcd) && lcdp->pwm_info.dev)
		return pwm_enable((struct pwm_device *)lcdp->pwm_info.dev);
...
static s32 disp_lcd_pwm_disable(struct disp_lcd *lcd)
{
	...
	if (disp_lcd_is_used(lcd) && lcdp->pwm_info.dev)
		pwm_disable((struct pwm_device *)lcdp->pwm_info.dev);
...
static s32 disp_lcd_set_bright(struct disp_lcd *lcd, u32 bright)
{
	...
	if ((lcdp->lcd_cfg.backlight_bright == 0) && (bright != 0))
		need_enable_bl = true;
	if ((lcdp->lcd_cfg.backlight_bright != 0) && (bright == 0))
	...
	if (lcdp->pwm_info.dev) {
		...
		pwm_config((struct pwm_device *)lcdp->pwm_info.dev,
			   duty_ns, period_ns);
	}
...
static s32 disp_lcd_init(struct disp_lcd *lcd)
{
	...
	if (lcdp->pwm_info.dev) {
		...
		pwm_set_polarity((struct pwm_device *)lcdp->pwm_info.dev,
