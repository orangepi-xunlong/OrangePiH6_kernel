...
s32 drv_disp_vsync_event(u32 sel)
{
	...
	if (!full) {
		g_fbi.vsync_timestamp[sel][head] = now;
		g_fbi.vsync_timestamp_head[sel] = next;
	}
...
static int vsync_proc(u32 disp)
{
	...
	while (tail != head) {
		time = g_fbi.vsync_timestamp[disp][tail];
		next = tail + 1;
		next = (next >= VSYNC_NUM) ? 0 : next;
		g_fbi.vsync_timestamp_tail[disp] =  next;
...
static int Fb_copy_boot_fb(u32 sel, struct fb_info *info)
{
	...
	src_addr = (char *)Fb_map_kernel_cache(
		(unsigned long)src_phy_addr - map_offset,
		src_stride * src_height + map_offset);
	if (NULL == src_addr) {
	...
	if (src_height < dst_height) {
		...
		__wrn("src_height(%d) is larger than dst_height(%d), "
			"you need cut the bootlogo height!\n", src_height, dst_height);
	...
	if (src_width < dst_width) {
		...
		__wrn("src_width(%d) is larger than dst_width(%d), "
			"you need cut the bootlogo width!\n", src_width, dst_width);
		Fb_unmap_kernel(src_addr);
		return -1;
	}
