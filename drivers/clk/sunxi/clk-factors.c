...
static int sunxi_clk_enable_pll_stable(struct sunxi_clk_factors *factor)
{
	...
	/* config a larger factor n value */
	if (config->nwidth)
...
static int sunxi_clk_fators_enable(struct clk_hw *hw)
{
...
#if (defined CONFIG_ARCH_SUN50IW3P1)
	if (sunxi_clk_enable_pll_stable(factor)) {
		if (factor->lock)
	...
	/* get factor register value */
	reg = factor_readl(factor, factor->reg);
	/* write the original n to factor register */
	if (config->nwidth)
