...
int sunxi_tsc_enable_hw_clk(void)
{
	...
	if (tsc_devp->mclk->enable_count) {
		TSC_MSG("clk is already enabled!\n");
		goto enable_out;
	}
...
int sunxi_tsc_disable_hw_clk(void)
{
	...
	spin_lock_irqsave(&tsc_devp->lock, flags);
	if (tsc_devp->mclk->enable_count == 0) {
		TSC_MSG("clk is already disabled!\n");
		goto disable_out;
	}
...
static int sunxi_tsc_hw_clk_init(struct platform_device *pdev)
{
	...
	if (ret) {
		...
		mclk_rate = clk_get_rate(tsc_devp->pclk);
		mclk_rate /= 2;
	}
	/* reset tsc module. */
...
static int sunxi_tsc_get_port_config(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		TSC_ERR("configuration missing or invalid.\n");
		ret = -EINVAL;
		return ret;
	} else {
