...
static int car_reverse_probe(struct platform_device *pdev)
{
	...
	if (gpio_request(car_reverse->reverse_gpio, NULL) != 0) {
		pr_warn("gpio request failed !\n");
		return -EINVAL;
	}

	if (gpio_set_debounce(car_reverse->reverse_gpio, 0x1) < 0) {
		dev_err(&pdev->dev, "failed to gpio_set_debounce");
	}

	/* gpio_free(car_reverse->reverse_gpio); */
