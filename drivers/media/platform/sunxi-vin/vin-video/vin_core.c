...
struct vin_debugfs_buffer {
	size_t count;
	char data[VIN_DEBUGFS_BUF_SIZE];
};
struct dentry *vi_debugfs_root;
...
static size_t vin_status_dump(struct vin_core *vinc, char *buf, size_t size)
{
	...
	switch (vinc->vin_status.pix_fmt) {
...
unlock:
	...
	if (cap->capture_mode == V4L2_MODE_IMAGE && status.capture_done) {
		...
		if (cap->first_flag == 0) {
			cap->first_flag++;
			v4l2_get_timestamp(&cap->ts);
			vin_print("capture video mode, first frame done!\n");
		}
		...
		/* video buffer handle */
		if ((&cap->vidq_active) == cap->vidq_active.next->next->next) {
			vin_warn("Only two buffer left for video%d\n", vinc->id);
			goto unlock;
		}
		buf = list_entry(cap->vidq_active.next, struct vin_buffer, list);
		...
		/* Nobody is waiting on this buffer */
		if (!waitqueue_active(&buf->vb.vb2_queue->done_wq)) {
			vin_warn("Nobody is waiting on video%d buffer%d\n",
...
static int vin_core_probe(struct platform_device *pdev)
{
	...
	vin_print("pdev->id = %d\n", pdev->id);
	vin_print("rear_sensor_sel = %d\n", vinc->rear_sensor);
	vin_print("front_sensor_sel = %d\n", vinc->front_sensor);
	vin_print("csi_sel = %d\n", vinc->csi_sel);
