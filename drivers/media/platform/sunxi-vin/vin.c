...
static int vin_md_clk_enable(struct vin_md *vind)
{
	...
	if (vind->clk[VIN_TOP_CLK].clock) {
		if (clk_set_rate(vind->clk[VIN_TOP_CLK].clock,
		    vind->clk[VIN_TOP_CLK].frequency)) {
			vin_err("set vin top clock rate error\n");
			return -1;
		}
		vin_print("vin top clk get rate = %ld\n",
	...
	vin_print("directly write pin and clk config @ FPGA\n");
...
static void vin_md_set_power(struct vin_md *vind, int on)
{
	...
	if (on) {
		...
		for (i = 0; i < VIN_MAX_DEV; i++) {
			...
			if (NULL == vinc)
				continue;
			csic_isp_input_select(vind->id, vinc->isp_sel, 0,
					vinc->csi_sel, 0);
			csic_vipp_input_select(vind->id, vinc->vipp_sel,
					vinc->isp_sel, 0);
		}
	} else {
	...
	for (num = 0; num < VIN_MAX_DEV; num++) {
		...
		for (i = 0; i < MAX_GPIO_NUM; i++) {
			...
			if (gpio_request(gc->gpio, NULL) < 0) {
				vin_warn("gpio%d request failed!\n", gc->gpio);
				continue;
			}
			vin_print("gpio%d request success!\n", gc->gpio);
		}
	}
#endif
	return 0;
				...
				struct media_entity *me, bool prepare)
{
	...
	if (get_sensor_info(sd->name, &sensor) == 0)
		vind->clk[VIN_TOP_CLK].frequency = sensor.core_clk_for_sensor;
	...
	if (type == VIN_MODULE_TYPE_CCI) {
		...
		if (IS_ERR_OR_NULL(sd)) {
			...
			if (v4l2_device_register_subdev(v4l2_dev, sd)) {
				...
				vin_print("%s register failed!\n", name);
				return NULL;
			}
			vin_print("%s register OK!\n", name);
		}
...
static int vin_probe(struct platform_device *pdev)
{
	...
	for (num = 0; num < VIN_MAX_DEV; num++) {
		...
		vind->modules[num].modules.flash.type = type;
