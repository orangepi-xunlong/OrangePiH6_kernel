    ...
    enum v4l2_colorfx value)
{
	...
	sensor_write_array(sd, sensor_colorfx[value].regs, sensor_colorfx[value].size);
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	SENSOR_ID |= (val<< 8);
	if (ret < 0) {
		sensor_err("sensor_err err at sensor_detect!\n");
		return ret;
	}
	
	ret = sensor_read(sd, 0xf1, &val);
	SENSOR_ID|= (val);
	sensor_print("V4L2_IDENT_SENSOR=%x",SENSOR_ID);
	if (ret < 0) {
		sensor_err("sensor_err err at sensor_detect!\n");
		return ret;
	}
