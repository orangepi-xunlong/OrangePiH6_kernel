...
#define  NAND_DRV_DATE			0x20170124
...
int NAND_GetVoltage(void)
{
	...
	if (IS_ERR(regu1))
		...
		if (IS_ERR(regu1)) {
			NAND_Print("nand:some err happen,fail to enable regulator vcc-nand!\n");
			return -1;
		}
	...
	if (IS_ERR(regu2))
		...
		if (IS_ERR(regu2)) {
			NAND_Print("nand:some err happen,fail to enable regulator vcc-io!\n");
			return -1;
		}
...
void Dump_Gpio_Reg_Show(void)
{
	...
	NAND_Print_DBG("Reg 0x01c20848: 0x%x\n",
	       *((volatile __u32 *)gpio_ptr + 18));
	NAND_Print_DBG("Reg 0x01c2084c: 0x%x\n",
	       *((volatile __u32 *)gpio_ptr + 19));
	NAND_Print_DBG("Reg 0x01c20850: 0x%x\n",
	       *((volatile __u32 *)gpio_ptr + 20));
	NAND_Print_DBG("Reg 0x01c2085c: 0x%x\n",
	       *((volatile __u32 *)gpio_ptr + 23));
	NAND_Print_DBG("Reg 0x01c20864: 0x%x\n",
...
void Dump_Ccmu_Reg_Show(void)
{
	...
	NAND_Print_DBG("Reg 0x01c20028: 0x%x\n",
	       *((volatile __u32 *)ccmu_ptr + 10));
	NAND_Print_DBG(KERN_EMERG "Reg 0x01c20080: 0x%x\n",
